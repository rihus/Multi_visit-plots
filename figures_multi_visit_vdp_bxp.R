...
df_2visits <- combined_spir_data %>%
  group_by(Subject_id) %>%
  filter(all(c("Visit1", "Visit2") %in% VISIT)) %>%
  ungroup() %>%
  filter(VISIT %in% c("Visit1", "Visit2"))
df_2visits_N4 <- df_2visits[df_2visits$Correction == "N4", ]
df_2visits_FA <- df_2visits[df_2visits$Correction == "FA", ]
# df_2visits_FA <- df_2visits_FA[-c(47, 48), ]

##Get data for all participants with 3 visits
df_3visits <- combined_spir_data %>%
  group_by(Subject_id) %>%
  filter(all(c("Visit1", "Visit2", "Visit3") %in% VISIT)) %>%
  ungroup() %>%
  filter(VISIT %in% c("Visit1", "Visit2", "Visit3"))

df_3visits_N4 <- df_3visits[df_3visits$Correction == "N4", ]
df_3visits_FA <- df_3visits[df_3visits$Correction == "FA", ]
...
###############################################2 visits
############# N4-corrected #################
visVDP_mean <- aggregate(VDP ~  VISIT, df_2visits_N4, mean)
visVDP_mean
visVDP_sd <- aggregate(VDP ~  VISIT, df_2visits_N4, sd)
visVDP_sd
#Correction       VDP
#Visit1         9.344125
#Visit2         7.574875
