...
ggsave("./zR_plots_4ppr/vdp_N4_2visits_cbxp_nop.png", plot = vdp_n4_bxp_nop, width = 4.5, height = 3.7, dpi = 300)
ggsave("./zR_plots_4ppr/vdp_N4_2visits_cbxp_p.png", plot = vdp_n4_bxp_p, width = 4.5, height = 3.7, dpi = 300)
############ FA-corrected ##################
visVDP_mean <- aggregate(VDP ~  VISIT, vdp_2visits_FA, mean)
visVDP_mean
visVDP_sd <- aggregate(VDP ~  VISIT, vdp_2visits_FA, sd)
visVDP_sd
#Correction       VDP
#Baseline         7.718083
#1year         6.471625
...
vdp_bxp_p <-  vdp_bxp_nop + stat_pvalue_manual(stat.test, label = "P={scales::pvalue(p)}",
                                               y.position = 30, label.size = 8, bracket.size = 0.8,
...
df_2o3_visits <- combined_spir_data %>%
  group_by(Subject_id) %>%
  filter(all(c("Baseline", "1year") %in% VISIT)) %>%
  filter(any(c("2year") %in% VISIT) | VISIT %in% c("Baseline", "1year")) %>%
  ungroup()
...
# #Box plots with p-values VDP - FA
vdp_bxp_nop <- ggpaired(vdp_3visits_FA, x = "VISIT", y = "VDP", fill = "VISIT",
                        palette = c("#004166", "#0072b2", "#00a3ff"), id = "Subject_id", width = 0.5,
                        ylim = c(0, 40), line.color = "black", line.size = 0.5,
                        legend = "none", xlab = "") +
  ylab(y_label) +
